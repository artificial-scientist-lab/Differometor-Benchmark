...
class OptimizationAlgorithm(ABC):
    ...
    Defines the core ask-tell interface for optimization algorithms.
    ...
    Note:
        Ask-tell algorithms implement:
        - `__init__(...)`: Initialize algorithm-specific meta-parameters
        - `ask(...)`: Propose one or more candidate parameter configurations
        - `tell(...)`: Receive evaluated losses and update optimizer state

        The Objective contains all evaluation-side run data:
    ...
    def ask(
        ...
        Returns:
            Array of shape (n_samples, n_params) containing candidate parameters.
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement ask()."
        )
    ...
    def tell(
        ...
        **kwargs,
        ...
        """Update the optimizer with evaluated candidates.

        Args:
            params: Candidate parameter vector(s) previously returned by ask().
            loss: Loss value(s) produced by evaluating ``params`` through Objective.
            **kwargs: Optional optimizer-specific feedback such as gradients,
                constraints, or acquisition metadata.
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement tell()."
        )
    ...
    #     Configures `obj.unbounded`, `obj.algorithm_str`, attaches a fresh
    #     OptimizationState, and seeds all relevant RNG sources (`np.random`, JAX).
    ...
    #     obj.initialize_optimization_state(
    #         algorithm_name=obj.algorithm_str,
    #         seed=random_seed,
    #         unbounded=unbounded,
    #     )
