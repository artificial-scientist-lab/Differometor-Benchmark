"""Mesh Adaptive Direct Search (MADS) algorithm via OMADS.

MADS is a derivative-free, direct-search method designed for rugged-landscape
local exploration.  It extends the GPS (Generalized Pattern Search) framework
by allowing an asymptotically dense set of poll directions, giving it
theoretical convergence guarantees on Lipschitz-continuous objectives.

This implementation uses the **GMESH** (generalised mesh) back-end in OMADS,
which realises the standard MADS poll step.

Bounded vs unbounded behaviour
--------------------------------
MADS operates exclusively in **bounded physical space** (``unbounded=False``).
...
>>> optimizer = MADS(poll_size_init=0.5)
...
class MADS(OptimizationAlgorithm):
    """Mesh Adaptive Direct Search (MADS) optimizer.

    Uses the OMADS library (GMESH back-end) to run the MADS poll step as a
    rugged-landscape local explorer.  Best suited for problems where gradients
    are unavailable, noisy, or discontinuous.

    Attributes:
        algorithm_str (str): ``"mads"``
    ...
    >>> MADS(poll_size_init=0.5).optimize(obj, random_seed=0)
    """
