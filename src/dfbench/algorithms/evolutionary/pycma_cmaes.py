...
Bounded behaviour
-----------------
All variants operate exclusively in bounded parameter space by default.
pycma's built-in boundary handling is enabled so the CMA mean stays close to
the feasible region; candidate solutions are additionally hard-clipped to the
problem's bounds before being passed to the Objective so evaluations are always
feasible.
...
def _ask_eval_tell(
    ...
    """Run one ask-clip-evaluate-tell generation through *obj*.

    Clips every candidate to ``[lb, ub]`` before calling ``obj.vmap_value``
    so no infeasible points are logged.  pycma's covariance update uses the
    clipped solutions, which is consistent with the evaluation.

    Args:
        es: Active ``CMAEvolutionStrategy`` instance managed by the caller.
        obj: Objective wrapper; logs all evaluations and manages the budget.
        lb_np: Lower bounds (numpy).
        ub_np: Upper bounds (numpy).
        batch_size: Max candidates per ``vmap_value`` call.
    """
    solutions = es.ask()
    clipped = [np.clip(x, lb_np, ub_np) for x in solutions]
    batch = jnp.asarray(np.stack(clipped))
    losses_jnp = _eval_batched(batch, obj, batch_size)
    es.tell(clipped, np.asarray(losses_jnp).tolist())
...
class PyCMACMAES(OptimizationAlgorithm):
    ...
    Bounded behaviour
    -----------------
    Operates exclusively in bounded parameter space.  pycma's boundary
    handling keeps the mean inside bounds; candidates are hard-clipped before
    evaluation.
    ...
    def optimize(
        ...
        opts = _build_opts(lb_np, ub_np, pop_size)
        ...
        while not es.stop() and not obj.budget_exceeded:
            if max_iterations is not None and iteration >= max_iterations:
                break
            _ask_eval_tell(es, obj, lb_np, ub_np, self._batch_size)
            iteration += 1
...
class PyCMAActiveCMAES(OptimizationAlgorithm):
    ...
    Bounded behaviour
    -----------------
    Identical to :class:`PyCMACMAES` (bounded by default, hard-clipped).
    ...
    def optimize(
        ...
        sigma = sigma0 if sigma0 is not None else float(np.mean(0.3 * (ub_np - lb_np)))
        pop_size = pop_size or _default_pop_size(n)
        opts = _build_opts(lb_np, ub_np, pop_size, active=True)
...
class PyCMAIPOP(OptimizationAlgorithm):
    ...
    def optimize(
        ...
        Args:
            ...
            random_seed: Seed for reproducibility.
            sigma0: Initial step size for each restart.  Defaults to
                ``0.3 × mean(ub − lb)``.
        ...
        sigma_base = sigma0 if sigma0 is not None else float(
            np.mean(0.3 * (ub_np - lb_np))
        )
        ...
        for restart in range(max_restarts + 1):
            ...
            opts = _build_opts(lb_np, ub_np, cur_pop)
            es = cma.CMAEvolutionStrategy(x0.tolist(), sigma_base, opts)
    ...
    * **Small population** – population size is drawn uniformly at random
    ...
    def optimize(
        ...
        Args:
            ...
            random_seed: Seed for reproducibility.
            sigma0: Base step size.  Defaults to ``0.3 × mean(ub − lb)``.
