...
class ReSTIR(OptimizationAlgorithm):
    ...
    def optimize(
        ...
        knn_reference_points = obj.random_params_bounded(n_samples=n_initial_reference_samples)
        ...
        for i in range(0, n_initial_reference_samples, self.batch_size):
            ...
            knn_reference_losses = knn_reference_losses.at[i:end].set(obj.vmap_value(batch))
        ...
        while not obj.budget_exceeded:
            ...
            all_losses, all_samples, is_predicted = (
                all_losses[perm], all_samples[perm], is_predicted[perm]
            )
            ...
            r_losses = jnp.pad(all_losses, (0, n_pad)).reshape(n_reservoirs, reservoir_size)
            r_samples = jnp.pad(all_samples, ((0, n_pad), (0, 0))).reshape(n_reservoirs, reservoir_size, -1)
            r_is_pred = jnp.pad(is_predicted, (0, n_pad)).reshape(n_reservoirs, reservoir_size)
            
            # Valid-entry mask (last reservoir may be partial)
            sizes = jnp.full(n_reservoirs, reservoir_size).at[-1].set(reservoir_size - n_pad) # Always reservoir_size except last one
            valid_mask = jnp.arange(reservoir_size)[None, :] < sizes[:, None] # (n_reservoirs, reservoir_size)
            ...
            sel_samples = r_samples[reservoir_indices, r_selected_idx]    # (n_reservoirs, D)
            sel_losses = r_losses[reservoir_indices, r_selected_idx]      # (n_reservoirs,)
            sel_is_pred = r_is_pred[reservoir_indices, r_selected_idx]    # (n_reservoirs,)
            ...
            if n_pred > 0:
                ...
                evaluated = jnp.concatenate([
                    obj.vmap_value(pred_samples[i:min(i + self.batch_size, n_pred)])
            ...
            if n_final_candidates > 0 and gd_steps > 0:
                ...
                all_ref_points = jnp.concatenate([knn_reference_points, gd_samples], axis=0)
                all_ref_losses = jnp.concatenate([knn_reference_losses, gd_losses], axis=0)
