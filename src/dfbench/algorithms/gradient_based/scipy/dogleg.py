...
class Dogleg(ScipyMinimizeAlgorithm):
    """SciPy dogleg using exact dense JAX Hessians in unbounded space."""

    algorithm_str = "dogleg"
    scipy_config = SciPyConfig(
        method="dogleg",
        unbounded=True,
        use_bounds=False,
        use_dense_hessian=True,
    )
    ...
    def optimize(
        ...
        **scipy_kwargs,
        ...
        """Run SciPy ``method="dogleg"`` with exact dense JAX Hessians.

        Dogleg is executed in dfbench's unbounded sigmoid space and receives an
        exact dense Hessian from JAX on every SciPy request.
        ...
        Args:
            ...
            **scipy_kwargs: Additional SciPy dogleg option entries forwarded via
                ``options=...``.
        """
