...
Constrained problems also expose aux diagnostics: `obj.value_aux(params)` and `obj.vmap_value_aux(batch)` return `(loss, aux)` where `aux` carries the loss decomposition, a physical `is_feasible` flag, per-constraint violations, and the raw per-group power arrays. Enable per-field recording with `save` tokens like `is_feasible`, `power_values`, or the `aux` alias. See [Problems](docs/Problems.md) for the aux schema and [Objective API Reference](docs/Objective-API-Reference.md) for the full method set.
