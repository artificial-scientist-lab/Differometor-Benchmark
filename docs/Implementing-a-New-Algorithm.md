...
This starts the wall-clock timer. Objective-provided `warmup_*()` calls before
this line are free. Evaluation methods, raw callable getters, and
`log_evaluation()` raise `RuntimeError` until this line has run.
